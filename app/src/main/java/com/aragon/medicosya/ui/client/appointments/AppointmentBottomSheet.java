...
public class AppointmentBottomSheet extends BottomSheetDialogFragment {
    ...
    public interface Listener {
        void onAppointmentChanged(); // para que el fragment padre pueda refrescar la lista
    }
    ...
    private void bindAppointment(DocumentSnapshot doc) {
        ...
        Tasks.whenAll(tasks).addOnSuccessListener(aVoid -> {
            ...
            if (appointment.getNotesProvider() != null && !appointment.getNotesProvider().isEmpty()) {
                binding.tvNotes.setText(appointment.getNotesProvider());
            }
    ...
    private void performCancel() {
        setLoading(true);
        repo.cancelAppointment(appointmentId, aVoid -> {
            setLoading(false);
            Toast.makeText(requireContext(),
                    "Cita cancelada", Toast.LENGTH_SHORT).show();
